...
class LanguageModel
{
    ...
    static languages := [{ name: "English", code: "en", flag: "england.png"}
                       , { name: "Español", code: "es", flag: "spain.png"} ]

    __New(ByRef settingsModel){
        this.settings := settingsModel.values
        
        ;TODO: read from settings
        ;this.selected := this.settings.get(lang)
        if (!this.selected) {
            this.selected := this.CalculateLanguage()
            ;TODO: save to settings
            ;this.settings.set(lang, this.selected)
        }
        Global i18n := New i18n("i18n", this.selected)
    }

    Change(newLanguage){
        this.selected := newLanguage
        ;TODO: save to settings
        ;this.settings.set(lang, newLanguage)
        
        ;TODO: Restart all vs Update view. Controller shoud do it after calling this method
        ;Global i18n := New i18n("i18n", this.selected)
    }
	...
	CalculateLanguage()
	{
        ...
        gameWindowHwnd := WinExist(Constants.RaidWinTitle)
        if gameWindowHwnd {
            languageCode := this.getKeyboardLanguage(gameWindowHwnd)
            languageName := this.getNameFromCode(languageCode)
